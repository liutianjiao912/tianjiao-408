...
function scanDirectory(directory: string, fileType: string): string[] {
  ...
  return fileList
    .map((file) => {
      ...
      if (fileExtension === normalizedFileType) {
        return filePath
      }
      return null
    })
 ...
 * 清空目标目录
 */
async function clearTargetDirectory(): Promise<void> {
  try {
...
/**
 * 将 Markdown 文件转换为思维导图 HTML
 */
async function convertMarkdownToHtml(mdList: string[]): Promise<void> {
 ...
 * 移动 HTML 文件到目标目录
 */
async function moveHtmlFilesToTarget(): Promise<void> {
...
async function markMapMain(): Promise<void> {
  try {
    ...
    await convertMarkdownToHtml(mdList)

    // 第三步：移动HTML文件到目标目录
    await moveHtmlFilesToTarget()
